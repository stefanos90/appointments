		...
		if (isset($_GET['id']) && newUSerRestricions($sanitize)){ //check for valid input, not empty etc
			...
			if (!$sanitize) { //if any errors, sanitize has strings, so becomes true.
				...
				if ($user->checkAppExist($id)) {
					...
					if ($user->completeApp($id)) {
						echo '<div class="alert alert-success fade in">Appointment marked as completed</div>';
					}
					else{
						echo '<div class="alert alert-warning fade in">Error completing appointment</div>';
					}
				}
				else{
